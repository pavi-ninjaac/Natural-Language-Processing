...
# analyse the commondata type.
def Common_data_analysis(data, missing_value_highlight_threshold=10.0, display_df=True, only_show_missing=False):
    """
    ...
    missing_value_by_character =  [data.loc[data[col].isin(["?", " ", "/", "[", "]", "na", "null"]), col].value_counts().sum() for col in column]
    missing_value_percentage_by_char = [round(col_missing_count / total_samples * 100, 2) for col_missing_count in missing_value_by_character]
    # organise values.
...
def find_outlier_z_score_method(data, new_feature=False, col_name=None, return_limits=False):
    """ 
    ...
    if df.ndim == 1:
            # if the given data is features than handle diffrently. because it has no .values and .index function.
            lower_limit_each_feature_df = pd.DataFrame({"Feature": [col_name if col_name else 'Given feature'], "lower limit":lower_limit_each_feature})
            upper_limit_each_feature_df = pd.DataFrame({"Feature": [col_name if col_name else 'Given feature'], "upper limit":upper_limit_each_feature})
    else:
            lower_limit_each_feature_df = pd.DataFrame({"Features":lower_limit_each_feature.index, "lower limit": lower_limit_each_feature.values})
            upper_limit_each_feature_df = pd.DataFrame({"Features":upper_limit_each_feature.index, "upper limit": upper_limit_each_feature.values})
    
    if new_feature and return_limits:
        
        df["num_of_outliers"] = outlier_df.sum(axis= 1)
        return df, number_of_outlier_each_feature_df, lower_limit_each_feature_df, upper_limit_each_feature_df
    elif new_feature:
...
def find_outliers_iqr_method(data, new_feature=False, col_name=None, return_limits=False):
    """ Find the outliers in the given dataset. 
    :param data: dataset has number of features to find the outliers.
    :param new_feature: If True create a new feature in the dataFrame to indicate this sample has any outlier feature else do nothing.
    :param return_limit: If true return the upper and lower limits.
